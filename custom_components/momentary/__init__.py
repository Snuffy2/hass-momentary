...
from .const import CONF_ID_TOKEN, CONF_YAML_PRESENT, CONF_YAML_SWITCH, DOMAIN
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if entry.data.get(CONF_YAML_SWITCH, False):
        if not entry.data.get(CONF_YAML_PRESENT, False):
            ...
            # _LOGGER.debug("[YAML] entry_id: %s", entry.entry_id)
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    # _LOGGER.debug("After Unload. hass: %s", hass.data[DOMAIN])
