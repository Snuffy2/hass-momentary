...
class MomentaryConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            user_input.update({CONF_YAML_SWITCH: yaml_switch})
            if yaml_switch:
                user_input.update({CONF_YAML_PRESENT: True})
            else:
                user_input.update({CONF_ID_TOKEN: slugify(user_input.get(CONF_NAME))})
...
class MomentaryOptionsFlowHandler(OptionsFlow):
    ...
    async def async_step_init(self, user_input: dict[str, Any] | None = None) -> ConfigFlowResult:
        ...
        return self.async_show_form(
            step_id="init",
            data_schema=_build_user_input_schema(user_input=user_input, fallback=config),
            errors=errors,
        )
